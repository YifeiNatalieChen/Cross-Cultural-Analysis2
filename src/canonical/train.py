...
def dcca():
    ...
    for iteration in xrange(max_iterations):
        ...
        SigmaHat11RootInv = torch.inverse(
            U1.mm(torch.pow(torch.diag(S1), 0.5)).mm(torch.t(V1)))
        SigmaHat22RootInv = torch.inverse(
            U2.mm(torch.pow(torch.diag(S2), 0.5)).mm(torch.t(V2)))
        ...
        # print G
        Delta11 = - \
            SigmaHat11RootInv.mm(A).mm(torch.diag(D)).mm(
                torch.t(A)).mm(SigmaHat11RootInv)
        Delta12 = SigmaHat11RootInv.mm(A).mm(torch.t(G)).mm(SigmaHat22RootInv)
        Delta22 = - \
            SigmaHat22RootInv.mm(A).mm(torch.diag(D)).mm(
                torch.t(A)).mm(SigmaHat22RootInv)

        dH1 = torch.mul(torch.mul(Delta11.mm(H1_hat), 2) +
                        Delta12.mm(H2_hat), 1.0/(m - 1))
        dH2 = torch.mul(torch.mul(Delta22.mm(H2_hat), 2) +
                        Delta12.mm(H1_hat), 1.0/(m - 1))
...
def main():
    parser = argparse.ArgumentParser(
        description='Deep Canonical Correlation Analysis')
    parser.add_argument('--dataset', default='AlphaGo',
                        help='Default is AlphaGo')
    ...
    parser.add_argument('--checkpoint_dir',
                        default='./checkpoint',  help='default as checkpoint')
    parser.add_argument('--n_epochs', type=int, default=100,
                        help='number of epochs to train for')
    parser.add_argument('--gpu_num', type=int, default=1,
                        help='number of gpu you want to use')
    parser.add_argument('--loading_weights', type=int, default=0,
                        help='whether loading from existing weights')
    parser.add_argument('--optimizer', type=str, default='Adam',
                        help='Adam, AdaMax or SGD, default is Adam')
    parser.add_argument('--loss', type=str, default='L2',
                        help='L1 or L2, default is L2')
    parser.add_argument('--evaluate', type=int, default=0,
                        help='1 for evaluation')
